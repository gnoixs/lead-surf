    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (!!data.state && data.state == 1) {            //表示获取红包成功
                ...
                $('.nov_curtion .nov-center').css('background', 'url(images/week2/rp-open-1.png) no-repeat center bottom');
                if (data.type == 0) {
...
$('.choose').bind('click',function(){
    if($(this).attr('style').indexOf('display') == 0){
        location.href = 'http://www.example.com/?location=leli115';
    }else{
        location.href = 'http://www.example.com/?location=shunli115';
    }
})
